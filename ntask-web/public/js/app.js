...
var App = function () {
    function App(body) {
        ...
        this.singin = new _signin2.default(body);
        this.singup = new _signup2.default(body);
    }
    ...
    _createClass(App, [{
        ...
        key: 'singinEvents',
        value: function singinEvents() {
            var _this = this;

            this.singin.on("error", function () {
                return alert("erro na autenticação");
            });
            this.singin.on("signin", function (token) {
                localStorage.setItem("token", 'JWT ' + token);
                alert("autenticação com sucesso");
            });
            this.singin.on("signup", function () {
                return _this.singup.render();
            });
        }
    }, {
        key: 'singupEvents',
        value: function singupEvents() {
            var _this2 = this;

            this.singup.on("error", function () {
                return alert("erro no cadastro");
            });
            this.singup.on("signup", function (user) {
                alert(user.name + ' bem vindo, voc\xEA foi cadastrado com sucesso!');
                _this2.singin.render();
            });
        }
    }]);
...
var Singin = function (_NTask) {
    _inherits(Singin, _NTask);

    function Singin(body) {
        _classCallCheck(this, Singin);

        var _this = _possibleConstructorReturn(this, (Singin.__proto__ || Object.getPrototypeOf(Singin)).call(this));
    ...
    _createClass(Singin, [{
        ...
        value: function formSubmit() {
            ...
            form.addEventListener('submit', function (e) {
                e.preventDefault();
                var email = e.target.querySelector('[data-email]').value;
                var password = e.target.querySelector('[data-password]').value;
                var options = {
                    method: 'POST',
                ...
                _this2.request(options, function (err, res, data) {
        ...
        key: 'singupClick',
        value: function singupClick() {
            var _this3 = this;

            var singup = this.body.querySelector('[data-signup]');
...
var Singup = function (_NTask) {
    _inherits(Singup, _NTask);

    function Singup(body) {
        _classCallCheck(this, Singup);

        var _this = _possibleConstructorReturn(this, (Singup.__proto__ || Object.getPrototypeOf(Singup)).call(this));
    ...
    _createClass(Singup, [{
        ...
        value: function formSubmit() {
            ...
            form.addEventListener('submit', function (e) {
                e.preventDefault();
                var name = e.target.querySelector('[data-name]').value;
                var email = e.target.querySelector('[data-email]').value;
                var password = e.target.querySelector('[data-password]').value;
                var options = {
                    method: 'POST',
